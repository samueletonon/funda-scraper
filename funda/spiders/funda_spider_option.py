...
class FundaSoldSpider(CrawlSpider):
    ...
    name = "funda_spider_option"
    allowed_domains = ["funda.nl"]

    def __init__(self, place='amsterdam'):
        self.start_urls = ["http://www.funda.nl/koop/in-onderhandeling/%s/p%s/" % (place, page_number) for page_number in range(1,1001)]
        # self.start_urls = ["http://www.funda.nl/koop/verkocht/%s/p1/" % place]  # For testing, extract just from one page
        self.base_url = "http://www.funda.nl/koop/in-onderhandeling/%s/" % place
